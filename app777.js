...
/*
  ...
  .then((json) => json.forEach(item => buildBlock(item.id, item.title, item.completed)))
*/
  ...
  .then((response)=>{
	  ...
	  return (setTimeout(() => {response}, 1000));
	...
	.then((data => data.forEach(item => buildBlock(item.id, item.title, item.completed))))
...
function removeBlock(){
	...
	let ulrFetch = `https://jsonplaceholder.typicode.com/posts/${this.parentNode.id}`
	fetch(ulrFetch , {
  	method: 'DELETE',
});
...
function renderFetch(){
	json.forEach(item => buildBlock(item.id, item.title, item.completed));
...
*/
/*
function buildBlock(id, title, completed){
const buildBlock = template.cloneNode(true);
console.log(buildBlock)


buildBlock.removeAttribute("id");	
buildBlock.setAttribute("id", id);
buildBlock.querySelector('.title').textContent = title;
buildBlock.querySelector('.check')
buildBlock.querySelector('.delete').addEventListener("click", removeBlock);
